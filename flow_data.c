...
void initialise_flow_data_2d(const int local_nx, const int local_ny,
  ...
  allocate_data(&flow_data->momentum_x_flux_x, (local_nx + 1) * (local_ny + 1));
  allocate_data(&flow_data->momentum_x_flux_y, (local_nx + 1) * (local_ny + 1));
  allocate_data(&flow_data->momentum_y_flux_x, (local_nx + 1) * (local_ny + 1));
  allocate_data(&flow_data->momentum_y_flux_y, (local_nx + 1) * (local_ny + 1));
}
...
void deallocate_flow_data(FlowData* flow_data) {
  ...
  deallocate_data(flow_data->momentum_x_flux_x);
  deallocate_data(flow_data->momentum_x_flux_y);
  deallocate_data(flow_data->momentum_y_flux_x);
  deallocate_data(flow_data->momentum_y_flux_y);
}
